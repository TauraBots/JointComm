...
void Joint::setSocket(int socket){
  this->jointSocket = socket;
}

int Joint::getSocket(){
  return this->jointSocket;
}
    ...
    case TORQUE:
      {
        ...
        dxl_write_byte(this->jointSocket, this->jointId, 0x18, 1);
    ...
    case OFF:
      {
        ...
        dxl_write_byte(this->jointSocket, this->jointId, 0x18, 0);
...
void Joint::setMaxTorque(int torque)
{
  ...
  dxl_write_word(this->jointSocket, this->jointId, 34, torque);
}
...
int Joint::getMaxTorque()
{
  ...
  // in the motors
  return dxl_read_word(this->jointSocket, this->jointId, 34);
}
...
void Joint::readMotorAngle()
{
  ...
  int value = dxl_read_word(this->jointSocket, this->jointId, 36);
  if (dxl_get_result() == 1)
  {
...
void Joint::writeGoalAngle(float angle)
{
  ...
  dxl_write_word(this->jointSocket, his->jointId, 30, this->jointCenterValue + ANGLE2VALUE(angle));
}
